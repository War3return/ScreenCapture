    ...
    public static class Component
    {
        ...
        public struct WarcraftInfo
        {
            ...
            public Process Process
            {
                ...
                set
                {
                    ...
                    Reset();
                    if (value != null && value.MainWindowHandle != IntPtr.Zero)
                    {
                        try
                        {
            ...
            public void Reset()
            {
                ...
                GameDllOffset =
                StormDllOffset =
                Memory.ChannelChat.ChannelOffset =
                Memory.ChannelChat.MessageOffset =
                Memory.ControlDelay.Offset =
                Memory.LoadedFiles.Offset =
                Memory.Message.CEditBoxOffset =
                Memory.Message.MessageOffset =
                Memory.Message.SelectedReceiverOffset =
                Memory.Message.TargetReceiverOffset =
                Memory.States.OsTcpOffset =
                Memory.BanList.Offset = IntPtr.Zero;
            }
        }

        internal static void DirectPatch(IntPtr offset, params byte[] buffer)
            => WriteProcessMemory(Warcraft3Info.Handle, offset, buffer, buffer.Length, out _);
        ...
        internal static IntPtr SearchAddress(byte[] search, uint maxAdd, int offset, uint interval = 0x10000)
        {
            ...
            for (uint num = 0x10000; num <= maxAdd; num += interval)
            {
                ...
                if (ReadProcessMemory(Warcraft3Info.Handle, lpBaseAddress, lpBuffer, search.Length, out int innerNum)
                    && CompareArrays(search, lpBuffer, innerNum))
        ...
        internal static IntPtr SearchMemoryRegion(byte[] signature, int offset = 4, uint maxAdd = 0x70000000)
        {
            ...
            while (lpBaseAddress.ToInt32() < maxAdd)
            {
                ...
                if (info.State == MemState.MEM_COMMIT && (uint)info.Protect < 0x100)
                {
                    ...
                    if (ReadProcessMemory(Warcraft3Info.Handle, lpAddress, buffer, signature.Length, out _)
        ...
        internal static IntPtr FollowPointer(IntPtr offset, params byte[] signature)
        {
            ...
            offset = new IntPtr(BitConverter.ToInt32(buffer, 0));
            while (true)
            {
                buffer = Bring(offset, 4 + length);
                if (buffer == null) return IntPtr.Zero;
                if (CompareArrays(buffer.Skip(4).ToArray(), signature, length))
                    return offset;
                int newOffset = BitConverter.ToInt32(buffer, 0);
