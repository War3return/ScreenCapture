...
namespace epicro.Wc3.Memory
{
    public static class States
    {
        ...
        public static bool IsLobbyList
        {
            get
            {
                if (Message.CEditBoxOffset != IntPtr.Zero)
                {
                    byte[] buffer = new byte[2];
                    ReadProcessMemory(Warcraft3Info.Handle, Message.CEditBoxOffset + 0x6E6, buffer, 2, out _);
                    if (CompareArrays(buffer, ChatStartPattern, 2)) return true;
                }
                return false;
            }
        }
        ...
        public static bool IsChatBoxOpen
            ...
            && BitConverter.ToBoolean(Bring(GameDllOffset + 0xD04FEC, 4) ?? new byte[4], 0);
        ...
        public static MusicState CurrentMusicState
        {
            get
            {
                if (GameDllOffset != IntPtr.Zero)
                {
                    ...
                    if (a == null || b == null) return MusicState.None;
        ...
        public static bool IsInGame
        {
            get
            {
                ...
                return state == MusicState.InGameDefault
                    || state == MusicState.InGameCustom
        ...
        public static int PlayerCount
            => GetOsTcpOffset() ? BitConverter.ToInt32(Bring(OsTcpOffset + 0x340, 4) ?? new byte[4], 0) : 0;

        public static bool IsHostPlayer
            => GetOsTcpOffset()
            && BitConverter.ToInt32(Bring(OsTcpOffset + 0x214, 4) ?? new byte[4], 0) == 2;
    }
}
