...
namespace epicro.Wc3.Memory
{
    public static class MessageFrame
    {
        ...
        public static string GetLastLobbyChat()
        {
            ...
            int index = BitConverter.ToInt32(buffer, 0) - 1;
            if (index == 0x7F)
            {
                if (!DirectBring(TextArrayDoublePointer + 8, 4, out buffer)) return null;
                index = BitConverter.ToInt32(buffer, 0) - 1;
            }
            ...
            int len = BitConverter.ToInt16(buffer, 0) - 9;
            if (len <= 0) return null;
            if (!DirectBring(TextOffset, len, out buffer)) return null;
