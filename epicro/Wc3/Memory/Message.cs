...
namespace epicro.Wc3.Memory
{
    public static class Message
    {
        ...
        private static readonly byte[] SelectedReceiverPattern  = { 0xD3, 0xAF, 0x2A, 0x26, 0xD3 };
        private static readonly byte[] TargetReceiverPattern    = { 0xB0, 0x32, 0x5B, 0x2C, 0xB0 };
        ...
        public static string GetMessage()
        {
            if (GetOffset())
            {
                ...
                IntPtr addr = CEditBoxOffset + 0x88
                    + (Settings.IsAutoFrequency ? 0 : 0x110 * Settings.ChatFrequency);
                if (ReadProcessMemory(Warcraft3Info.Handle, MessageOffset = addr, buffer, 0x100, out _))
        ...
        public static async Task DetectChatFrequency()
        {
            ...
            byte[] bytes = Encoding.UTF8.GetBytes(
                $"\x1{Theme.MsgTitleColor}{Theme.MsgTitle} {Theme.MsgColor}채팅 주파수 검색 중...");
            for (int i = 0; i < 20; i++)
                WriteProcessMemory(Warcraft3Info.Handle,
                    CEditBoxOffset + 0x88 + 0x110 * i, bytes, bytes.Length + 1, out _);
            ApplyChat(false);
            await Task.Delay(200);
            for (int i = 0; i < 20; i++)
            {
                byte[] buf1 = new byte[1];
                if (ReadProcessMemory(Warcraft3Info.Handle,
                    CEditBoxOffset + 0x88 + 0x110 * i, buf1, 1, out _) && buf1[0] == 0)
                {
                    Settings.ChatFrequency = i;
                    for (int j = 0; j < 20; j++)
                        WriteProcessMemory(Warcraft3Info.Handle,
                            CEditBoxOffset + 0x88 + 0x110 * j, new byte[] { 0 }, 1, out _);
                    return;
                }
            }
        }
        ...
        public static bool GetSelectedReceiveStatus()
        {
            ...
            if (SelectedReceiverOffset != IntPtr.Zero)
            {
                ...
                if (ReadProcessMemory(Warcraft3Info.Handle,
                    SelectedReceiverOffset += 0x260, buffer, 4, out _))
                    return true;
            }
        ...
        {
            DirectPatch(Settings.IsAutoFrequency
                ? MessageOffset
                : (CEditBoxOffset + 0x88 + 0x110 * Settings.ChatFrequency), 0);
        }
        ...
        private static void MessageCut(string pMessage, bool IsHide)
        {
            ...
            if (Settings.IsAutoFrequency)
                ...
                WriteProcessMemory(Warcraft3Info.Handle,
                    CEditBoxOffset + 0x88 + 0x110 * Settings.ChatFrequency, bytes, bytes.Length + 1, out _);
        ...
        public static bool SendMsg(bool UseTitle, string[] args, int delay, bool IsHide = true)
        {
            ...
            foreach (var arg in args)
            {
                ...
                if (!string.IsNullOrEmpty(arg))
                    MessageCut((UseTitle
                        ? $"\x1{Theme.MsgTitleColor}{Theme.MsgTitle} {Theme.MsgColor}"
                        : string.Empty) + arg, IsHide);
            }
            return true;
        }
