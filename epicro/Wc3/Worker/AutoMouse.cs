        ...
        public static bool Enabled
        {
            ...
            set
            {
                if (_Enabled != value)
                {
                    if (value)
                    {
                        if (_LeftStartKey  != 0) hotkeyList.Register(_LeftStartKey,  OnLeft,  _LeftStartKey);
                        if (_RightStartKey != 0) hotkeyList.Register(_RightStartKey, OnRight, _RightStartKey);
                        hotkeyList.Register(_EndKey, Off, _EndKey);
                    }
                    else
                    {
                        if (hotkeyList.IsRegistered(_LeftStartKey))  hotkeyList.UnRegister(_LeftStartKey);
                        if (hotkeyList.IsRegistered(_RightStartKey)) hotkeyList.UnRegister(_RightStartKey);
