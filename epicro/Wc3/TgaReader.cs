...
namespace epicro.Wc3
{
    public static class TgaReader
    {
        ...
        public static Bitmap Load(Stream stream)
        {
            ...
            if (colorMap > 1) throw new ApplicationException("This is not a valid TGA file.");
            if (idFieldLength > 0)
            {
                        ...
                        else
                        {
                            ...
                            switch (bitsPerPixel)
                            {
                                case 8:
                                    ...
                                    break;
                                case 24:
                                    int r = stream.ReadByte(), g = stream.ReadByte(), b = stream.ReadByte();
                                    for (int j = 0; j < i; j++)
                                    {
                                    ...
                                    break;
                                case 32:
                                    int r2 = stream.ReadByte(), g2 = stream.ReadByte(), b2 = stream.ReadByte(), a2 = stream.ReadByte();
                                    for (int j = 0; j < i; j++)
                                    {
                                        bmpData[4*(y*imgWidth+x)]   = (byte)r2;
                                        bmpData[4*(y*imgWidth+x)+1] = (byte)g2;
                                        bmpData[4*(y*imgWidth+x)+2] = (byte)b2;
                                        bmpData[4*(y*imgWidth+x)+3] = 0xFF;
                                        x++; if (x >= imgWidth) { x = 0; y--; }
                                    }
                                    break;
            ...
            System.Drawing.Imaging.BitmapData bmpBits = theBitmap.LockBits(
            ...
            if (imgOrientation == 1) theBitmap.RotateFlip(RotateFlipType.RotateNoneFlipX);
            else if (imgOrientation == 2) theBitmap.RotateFlip(RotateFlipType.RotateNoneFlipY);
            else if (imgOrientation == 3) theBitmap.RotateFlip(RotateFlipType.RotateNoneFlipXY);

            return theBitmap;
        }

        private static UInt16 LittleEndian(UInt16 val) => BitConverter.IsLittleEndian ? val : conv_endian(val);
        private static UInt32 LittleEndian(UInt32 val) => BitConverter.IsLittleEndian ? val : conv_endian(val);

        private static UInt16 conv_endian(UInt16 val)
        {
            UInt16 temp = (UInt16)(val << 8); temp &= 0xFF00; temp |= (UInt16)((val >> 8) & 0xFF);
            return temp;
        }
